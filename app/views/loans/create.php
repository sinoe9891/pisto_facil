                  ...
                  <div class="card-body py-2">
                    <div class="d-flex align-items-center justify-content-between">
                      <div class="fw-semibold">
                        ...
                        Aval requerido para montos ≥ <?= $currency ?> <?= number_format($avalMin, 2) ?>
                      </div>
                    ...
                    <div class="row g-2 mt-2">
                      <div class="col-md-6">
                        <label class="form-label fw-semibold mb-1">
                          Seleccionar Aval <span class="text-danger">*</span>
                        </label>
                        <select name="aval_id" id="aval_id" class="form-select">
                          <option value="">-- Seleccionar aval --</option>
                        </select>
                        <div class="form-text">Solo aparecen avales del cliente seleccionado.</div>
                      </div>

                      <div class="col-md-6" id="aval_quick" style="font-size:.85rem">
                        <div class="text-muted">Seleccione un aval para ver detalle.</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="row g-2 mb-3" id="liveSummary">
                <div class="col-3">
                  <div class="card border-0 bg-light p-2 text-center">
                    <div class="fw-bold text-primary" id="sum_payment">—</div>
                    <div class="text-muted" style="font-size:.7rem">Cuota</div>
                  </div>
                </div>
                <div class="col-3">
                  <div class="card border-0 bg-light p-2 text-center">
                    <div class="fw-bold text-danger" id="sum_interest">—</div>
                    <div class="text-muted" style="font-size:.7rem">Total Interés</div>
                  </div>
                </div>
                <div class="col-3">
                  <div class="card border-0 bg-light p-2 text-center">
                    <div class="fw-bold text-success" id="sum_total">—</div>
                    <div class="text-muted" style="font-size:.7rem">Total a Pagar</div>
                  </div>
                </div>
                <div class="col-3">
                  <div class="card border-0 bg-light p-2 text-center">
                    <div class="fw-bold text-warning" id="sum_lastdate">—</div>
                    <div class="text-muted" style="font-size:.7rem">Último Pago</div>
                  </div>
                </div>
              </div>
              <!-- TABLE -->
...
<script>
  ...
  // ─── TABLA EN VIVO ────────────────────────────────────────────
  function fmt(n) {
    return CUR + ' ' + parseFloat(n || 0).toLocaleString('es-HN', {
  ...
  async function loadAvalsForClient(clientId) {
    ...
    if (!clientId) {
      quick.innerHTML = `<div class="text-muted">Seleccione un cliente primero.</div>`;
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        quick.innerHTML = `<div class="text-danger">Error cargando avales (HTTP ${res.status}).</div>`;
        return;
      }
      ...
      data.forEach(a => {
        ...
        opt.value = a.id;
        opt.textContent = `${a.full_name} · ${a.identity_number}`;
        opt.dataset.phone = a.phone || '';
        opt.dataset.rel = a.relationship || '';
        sel.appendChild(opt);
      });

      quick.innerHTML = `<div class="text-muted">Avales disponibles: <strong>${data.length}</strong></div>`;
    } catch (e) {
      quick.innerHTML = `<div class="text-danger">Error cargando avales.</div>`;
    }
  }

  function updateAvalBlock() {
    const P = parseFloat(document.getElementById('principal')?.value) || 0;
    const block = document.getElementById('aval_block');
    const sel = document.getElementById('aval_id');
    const quick = document.getElementById('aval_quick');

    if (!block || !sel || !quick) return;

    const required = (AVAL_MIN > 0 && P >= AVAL_MIN);

    block.classList.toggle('d-none', !required);
    sel.required = required;

    if (!required) {
      sel.value = '';
      sel.classList.remove('is-invalid');
      quick.innerHTML = `<div class="text-muted">No se requiere aval para este monto.</div>`;
      return;
    }
  ...
  document.getElementById('payment_frequency').addEventListener('change', () => {
    refreshTable();
    updateTermEquiv();
  });
  ...
  document.getElementById('aval_id')?.addEventListener('change', () => {
    const sel = document.getElementById('aval_id');
    const quick = document.getElementById('aval_quick');
