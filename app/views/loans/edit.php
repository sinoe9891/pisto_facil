      ...
      <div class="card-body">
        ...
        <form method="POST" action="<?= url('/loans/' . $loan['id'] . '/update') ?>">
          ...
          <div class="row g-3">
            ...
            <!-- Editable -->
            <div class="col-md-4">
              <label class="form-label">Asesor Asignado <span class="text-danger">*</span></label>
              <select name="assigned_to" class="form-select" required>
                <option value="" disabled <?= empty($loan['assigned_to']) ? 'selected' : '' ?>>Seleccione un asesor...</option>
                <?php foreach ($advisors as $a): ?>
                  <option value="<?= (int)$a['id'] ?>" <?= ((int)$loan['assigned_to'] === (int)$a['id']) ? 'selected' : '' ?>>
                    <?= htmlspecialchars($a['name']) ?>
                  </option>
                <?php endforeach; ?>
              </select>
              <div class="invalid-feedback">Debe seleccionar un asesor.</div>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
              <label class="form-label">Tasa Moratoria (%/mes) <span class="text-danger">*</span></label>
              <input type="number" name="late_fee_rate" class="form-control"
                step="0.01" min="0" max="100" required
                value="<?= number_format($loan['late_fee_rate'] * 100, 2) ?>">
              <div class="invalid-feedback">Ingrese la tasa moratoria (puede ser 0).</div>
            </div>
            <div class="col-md-4">
              <label class="form-label">Días de Gracia <span class="text-danger">*</span></label>
              <input type="number" name="grace_days" class="form-control"
                min="0" max="30" required
                value="<?= (int)$loan['grace_days'] ?>">
              <div class="invalid-feedback">Ingrese los días de gracia (0 a 30).</div>
            </div>
            <div class="col-12">
