...
<div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <div class="btn-group btn-group-sm">
        <a href="#" onclick="confirmEditClient()" class="btn btn-outline-primary">
            <i class="bi bi-pencil me-1"></i>Editar
        </a>
        <a href="<?= url('/loans/create?client_id=' . $client['id']) ?>" class="btn btn-success">
            <i class="bi bi-plus me-1"></i>Nuevo Préstamo
        </a>
        <a href="#" onclick="confirmDeleteClient()" class="btn btn-outline-danger">
...
<div class="row g-3">
    ...
    <div class="col-xl-8">
        ...
        <div class="card shadow-sm border-0 mb-3">
            <div
                class="card-header fw-semibold bg-white border-bottom py-2 d-flex justify-content-between align-items-center">
            ...
            <div class="card-body p-0">
                ...
                <div class="table-responsive">
                  ...
                  $statusMap = [
                    ...
                    'paid' => ['bg-primary', 'Pagado'],
                    'defaulted' => ['bg-danger', 'Moroso'],
                    'cancelled' => ['bg-secondary', 'Cancelado'],
                  ...
                  ?>
                            <tr>
                                <td><a
                                        href="<?= url('/loans/' . $l['id']) ?>"><?= htmlspecialchars($l['loan_number']) ?></a>
                                </td>
                                <td><span class="badge bg-info text-dark">Tipo <?= $l['loan_type'] ?> ·
                                        <?= $typeMap[$l['loan_type']] ?? '' ?></span></td>
                                <td class="text-end"><?= $currency ?> <?= number_format($l['principal'], 2) ?></td>
                                <td class="text-end fw-semibold"><?= $currency ?> <?= number_format($l['balance'], 2) ?>
                                </td>
        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body">
                ...
                <div class="text-center text-muted py-3"><i class="bi bi-folder-x d-block fs-3 mb-1"></i>Sin documentos.
                </div>
                <?php endif; ?>
                        ...
                        <div class="col-sm-6 col-md-4">
                            <div class="border rounded p-2 d-flex align-items-center gap-2" style="font-size:.8rem">
                                ...
                                <div class="d-flex flex-column gap-1">
                                    ...
                                    <?php if ($isAdmin): ?>
                                    <a href="#"
                                        onclick="confirmDelete('<?= url('/clients/' . $client['id'] . '/doc/' . $doc['id'] . '/delete') ?>')"
...
<div class="modal fade" id="uploadModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="<?= url('/clients/' . $client['id'] . '/upload') ?>"
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">PDF, JPG, PNG, WEBP · Máx. <?= setting('max_upload_size_mb', 10) ?> MB
                        </div>
                    </div>
                    <div class="mb-0">
...
function confirmEditClient() {
    Swal.fire({
        ...
        html: 'Vas a editar a <strong><?= htmlspecialchars($client['first_name'] . ' ' . $client['last_name']) ?></strong>.<br>' +
            'Los cambios afectarán todos sus préstamos y documentos vinculados.',
        icon: 'info',
        ...
        if (r.isConfirmed) window.location.href = '<?= url('/clients/' . $client['id'] . '/edit') ?>';
    });
}
...
function confirmDeleteClient() {
    Swal.fire({
        ...
        html: '<strong><?= htmlspecialchars($client['first_name'] . ' ' . $client['last_name']) ?></strong> ' +
            'será marcado como <span class="badge bg-secondary">Inactivo</span>.<br>' +
        ...
        if (r.isConfirmed) window.location.href = '<?= url('/clients/' . $client['id'] . '/delete') ?>';
    });
}
</script>